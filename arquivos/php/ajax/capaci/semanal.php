...
<?php	
	$sql = $mysqli->query("SELECT * FROM capacitacao WHERE semana='$semana' AND ano='$ano' ORDER BY id");
	while($aulas = $sql->fetch_array()) {
	$instrutor = $mysqli->query("SELECT * FROM usuarios WHERE id='".$aulas['capacitador_id']."'")->fetch_array();
?>
			<tr>
				...
				<td><div class="botao vermelho" onclick="capaci.excluir('<?php echo $aulas['id']; ?>','semanal');">Excluir</div></td>
			</tr>
<?php } ?>
		</table>
	</div>
