	...
	<div class="corpo">
		<table class="perfil">
			<tr>
				...
				<td>DATA E HORA DO INÍCIO DO TESTE</td>
				<td>APLICADOR</td>
...
<?php	
	$sql = $mysqli->query("SELECT * FROM admissao WHERE semana='$semana' AND ano='$ano' ORDER BY id");
	while($aulas = $sql->fetch_array()) {
		if($aulas['tipo'] == 0) {
			$admit = "Aconteceu um erro";
		}elseif($aulas['tipo'] == 1) {
			$admit = "Foi capacitado";
		}elseif($aulas['tipo'] == 2) {
			$admit = "Não foi capacitado";
		}
	$instrutor = $mysqli->query("SELECT * FROM usuarios WHERE id='".$aulas['aplicador_id']."'")->fetch_array();
?>
			<tr>
				...
				<td><div class="botao vermelho" onclick="admi.excluir('<?php echo $aulas['id']; ?>','semanal');">Excluir</div></td>
			</tr>
<?php } ?>
		</table>
	</div>
